...
**Architecture:** one `iroh::endpoint::EndpointHooks` impl (`MeshHooks`) installed in `build_endpoint`. `before_connect` vetoes refused ids on every ALPN except `ALPN_PAIR`; `after_handshake` registers a `WeakConnectionHandle` per remote id (outbound only, non-pair — see the review note below) and re-checks outbound dials. Every revoke path runs one close pass over that registry. Based on #223 (`dial::refused_by`).
...
- **TOCTOU:** outbound `after_handshake` INSERTS first, then re-checks `refused_by`, removing + rejecting (`CLOSE_UNAUTHORIZED`, `b"revoked on this node"`) on refusal. A revoke write before the recheck ⇒ rejected; after ⇒ its close pass sees the entry.
...
- [x] **1. Fail-closed hook + install** — `node/src/daemon/hooks.rs` (new), `boot.rs::build_endpoint` gains `hooks: Option<MeshHooks>`. Test (unit): unset cell ⇒ `connect(.., ALPN_MCP)` is `LocallyRejected`; `ALPN_PAIR` dial connects. Mutation: `before_connect` → `Accept`.
- [x] **2. Refusal via cell** — set cell in `boot_node`; unit test with a revoked id refused, unrevoked accepted.
- [x] **3. Gossip learned-peer test** — A from `build_endpoint(roster, Some(hooks))` + gossip; B, Y, X raw gossip endpoints; X revoked in A's store. Y (control) and X join via B (ForwardJoin) and via `join_peers`; assert Y dialed/neighbour, X zero accepts from A per ALPN and never in `neighbors()`. Mutation: remove `.hooks(..)` in `build_endpoint`.
- [x] **4. Registry + prune** — `after_handshake` registers; test N open/close ⇒ registry empty (bounded wait).
- [x] **5. Close on revoke** — `MeshState::peer_hooks` (OnceLock, set at boot); close pass in `sever_principals` + `install_roster_view_and_sever`. Tests (embedded, `cli/tests/embedded_loopback.rs`): held `connect_protocol` conn closes after `peer_revoke`; `open_session` pipe EOFs after `peer_revoke`; roster install revoking closes too. Mutation: drop the close-pass call.
- [x] **6. Docs + API_MINOR 64 → 65** (new outbound refusals on embedder app protocols; open outbound connections now severed), history entry, `dial_refused` doc "Not covered" list updated.
- [x] **7. Verify** fmt, clippy, full suite; adversarial pass over `git diff cf78376..HEAD`.

## Review note (af38085)

Registering INBOUND connections as well made `severed` undercount: `sever_principals` runs the close
pass first, it cut the inbound session, the handler dropped its `ConnRegistry` registration while
principal resolution ran, and the inbound sever counted 0. The registry is now outbound-only; the
accept loop's `ConnRegistry` owns inbound connections (pinned by
`a_revoke_counts_the_inbound_session_it_severs`, 5 runs).
