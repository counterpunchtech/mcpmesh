...
| **Peer name** | The human handle for a remote identity: the locally-stored nickname (pairing mode) or a roster `user_id` (roster mode). In both modes a verified person-level `user_id` may ALSO be present (device→user bindings, D4) — the nickname names a device, the `user_id` names the person. |
...
State: a local, daemon-owned allowlist — `{ endpoint_id, nickname, services: [names], paired_at }` per peer, persisted in `state.redb`.

- `mcpmesh invite [services…]` mints `{ secret, this endpoint's dialable address **and EndpointId**, suggested nickname, granted services, expiry ≤ 24h, remaining uses }`, encoded as one `mcpmesh-invite:` line. The secret is a bearer credential: short-lived, exchanged out-of-band (chat, QR), and **single-use unless `--uses N` was given** (#87 — capped at 64, each redemption its own ceremony). Interception risk is bounded by the expiry, the remaining-use count, the bindings below, and the human noticing an unexpected "paired" notification (§11, P3); asking for more uses widens that bound proportionally, which is why it is opt-in and reported back.
- `mcpmesh pair <invite>` dials the inviter on ALPN `mcpmesh/pair/1`. The redeemer MUST verify that the TLS-authenticated peer EndpointId equals the invite's (closing address-swap substitution), then proves possession of the secret; the two daemons exchange identities and both display a **short authentication code** (a few words derived from a transcript hash over both EndpointIds + the secret) in the completion notice — "Paired with alice — code: tango-fig-42" — so a whole-invite forgery is catchable by a second-channel human check. Mutual entries are written; the invite's remaining-use count is decremented, and it is burned when that reaches zero (once, for an ordinary single-use invite). Failed secret verification never burns the invite; attempts are capped (small fixed budget per invite, e.g. 10, after which the invite is invalidated), each attempt logged as a trust event (§11.3), and the rendezvous listener is torn down at burn or expiry.
...
- Unpair: `mcpmesh org revoke` is roster-mode; pairing mode uses `mcpmesh pair --remove <nickname>` (drops the entry, disconnects live sessions).
- **Person identity in pairing mode (D4):** every daemon auto-mints a self-sovereign user key at boot; the rendezvous exchanges each side's **device→user binding** (a user-key signature over the device endpoint id, `trust/src/binding.rs`) and verifies it before storing. A paired peer therefore resolves with a VERIFIED `user_id` alongside its nickname — the same `PeerIdentity` shape as roster mode — so person-level grants (§5) and multi-device reach work without an org. An unbound legacy peer simply has `user_id: None`.
...
2. **Initialize-time (service check):** the session names a service (§7.2); the daemon checks the resolved identity against that service's `allow` list (nicknames, user_ids, group names — one flat namespace). Failure → JSON-RPC error `-32054`, worded identically to "no such service" (existence of unshared services is not disclosed).

**The principal set (normative, one implementation).** A resolved caller's flat authorization identity is `groups ∪ {nickname} ∪ {user_id}` (empty components skipped; no identity ⇒ empty set ⇒ default deny). This expansion has exactly ONE implementation — `mcpmesh_local_api::principal_set` (feature-free, dependency-free) — consumed by all three enforcement sites: the mesh service `allow` check (`mcpmesh-net`), the plugin seam's audience expansion (`peer_audiences`), and the blob-scope gate (§9). The nickname is a first-class principal at EVERY site, including blob scopes: a pairing-mode peer with no user binding can be granted — and must be admitted — by its nickname alone.
...
allow = ["bob"]                    # nicknames, user_ids, and/or groups
...
- **Trust & services:** trust operations (invite/pair/join/status), service management — including an application idempotently registering/updating its own `[services.*]` entry — and a status/event stream. Anything the porcelain can do, an API client can do, and vice versa. The person-centric contact ops live here too: `peer_remove` (drop a pairing + its `allow` grants) and `peer_rename` (authoritatively rename a contact — every device sharing a `user_id`, or a single provisional nickname — rewriting `[services.*].allow` so grants follow the rename; refuses a collision with a different identity). The host Contacts surface is a pure projection over `status` plus these ops.
...
2. **Request-time:** the serving app registers blobs under named **scopes** (e.g. the KB app: one scope per audience); the provider serves a hash only to callers whose identity is granted a scope containing it. Grants draw from the SAME §5 flat principal namespace — `groups ∪ {nickname} ∪ {user_id}`, via the one shared `principal_set` — so a scope granted to a pairing-mode nickname admits that peer exactly as a service `allow` would (the KB app's attachment scopes rely on this: its audiences include nicknames). Hashes are integrity proofs, never capabilities.
...
| P3 | Invite-code interception / substitution | Attacker redeems — or swaps — a pairing invite in transit | Single-use by default and ≤ 24 h expiry — or up to `max_uses` redemptions (capped 64, #87) when the minter asks, which widens the interception blast radius to that many pairings and is why it is opt-in, capped, and reported back; out-of-band channel choice; invite binds the inviter's EndpointId, verified against the TLS identity at redemption; short authentication code shown on both sides (§4.2); completion announced on the inviter's side; a hijacked invite grants only its listed services, unpairable on sight |
...
| P9 | Gossip abuse (roster mode) | Forged presence, topic spam, metadata injection | Signed heartbeats verified against roster; non-roster senders dropped; the optional app-metadata blob (#39) is inside the signature and capped ≤256 B on BOTH set and receive (an oversized beat is dropped at verify); rendered control-char-stripped |
