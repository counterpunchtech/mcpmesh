...
- `mcpmesh invite [services…]` mints `{ one-time secret, this endpoint's dialable address **and EndpointId**, suggested nickname, granted services, expiry ≤ 24h }`, encoded as one `mcpmesh-invite:` line. The secret is a bearer credential: single-use, short-lived, exchanged out-of-band (chat, QR) — interception risk is bounded by those properties, the bindings below, and the human noticing an unexpected "paired" notification (§11, P3).
