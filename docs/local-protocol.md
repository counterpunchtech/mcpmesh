...
| `invite` | `{services:[…]}` — mint a single-use pairing invite. **Outstanding invites survive a daemon restart** (#87b): they are persisted, so `expires_at_epoch` is the real lifetime rather than an upper bound on process lifetime. A mint that cannot be persisted is an ERROR rather than an invite that will quietly not survive. | `{invite_line:"mcpmesh-invite:…", expires_at_epoch}` |
