...
| `user_key_export` | *(none)* — `api_minor >= 48`, #85 | `{recovery_phrase, user_id}` — the user key as 33 words a person can write down. **The phrase IS the private key**: anyone who reads it can present this identity. It is deliberately absent from the audit log, from `status`, and from every other surface — this response is the only place it exists. `user_id` is safe to display and record; compare it after an import. **Refused (`-32602`) on an ENROLLED device** (#86, #219): the local key `boot` minted is not the identity this node presents, so its phrase would restore a stranger to every peer — export from the device that holds the key. |
