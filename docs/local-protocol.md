...
| `peer_introduce` | `{subject, endorsed_by, evidence, subject_user_id?, nickname}` — install a peer from a **signed endorsement** by someone you are already paired with (#65, `api_minor >= 42`). Onboards a small group in O(N) instead of O(N²) two-human ceremonies. **It installs IDENTITY and grants NOTHING** — the subject becomes resolvable; service access stays principal-keyed in config (#38) and an explicit, separate act. That is what bounds it: a compromised endorser can make you *know about* an attacker, never *serve* one. `endorsed_by` must be the `user_id` of a **currently** paired peer, so the trust chain terminates at a ceremony you performed yourself; unpairing them revokes their power to introduce, even though their old signature stays cryptographically valid. Refused for: an unknown/unpaired endorser, a signature naming a different subject, your own endpoint id, or a nickname you already use for a different peer. | `{}` |
...
That is a real reduction, and it is the point of the feature. What bounds it is that an introduction
buys **identity resolution only**. The subject is granted nothing, so the worst a compromised
endorser achieves is that your node knows a name it should not. Grant deliberately, as you would for
any peer.
...
- **`api_version` = `"MAJOR.MINOR"`, and `api_minor` = the integer MINOR** — the
  ...
  `api_minor >= 41`; the `peer_introduce` verb (#65) is `api_minor >= 42`, and being a server-PUSHED frame it simply never arrives below that — below that `deny_unknown_fields` rejects the whole request, so
