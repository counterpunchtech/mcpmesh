...
| `blob_fetch` | `{ticket, dest_path}` | `{hash, bytes_len}` **`from`** (`api_minor >= 47`, #83) names ADDITIONAL sources to try when the publisher does not answer — stable principals or paired nicknames, tried in order **after** the ticket's own address, so a live publisher costs nothing and an offline one costs one dial timeout. The bytes stay BLAKE3-verified against the ticket's hash whoever serves them, so an alternate can refuse but never substitute; it must have republished the hash into a scope that grants you. |
