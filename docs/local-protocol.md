...
- **`api_version` = `"MAJOR.MINOR"`, and `api_minor` = the integer MINOR** — the
  ...
  (`open_session`, `peer_services`, `peer_diagnostics`) — refuses any stored row carrying a
  revoked `user_id` — so a per-device `peer_unrevoke` under a standing identity revocation re-admits
