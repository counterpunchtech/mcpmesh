...
- **Connections this node DIALLED are closed only for a device it now refuses** (`api_minor >= 65`,
  ...
  pairing. Withdrawing a principal from one service's `allow` refuses no dial, so it leaves outbound
  connections alone. None of these are counted in `severed`.
...
- **`api_version` = `"MAJOR.MINOR"`, and `api_minor` = the integer MINOR** — the
  ...
  close connections this node OPENED to a device they revoke — an `open_session` pipe ends, a held
  `connect_protocol` connection closes with code 401. `severed` still counts only accepted
  connections. Guard on `>= 65` before relying on an outbound session ending when you revoke its
  peer.
