...
| `peer_services` | `{peer}` — discover which services a paired `peer` (nickname / `eid:` / `b64u:`) CURRENTLY grants you (#52): dials the peer over `mcpmesh/ping/1` and returns `{services:[…]}` — the names whose allow admits YOUR principal (only yours, never the peer's full registry). Refuses a REVOKED peer before dialling; from `api_minor >= 64` (#223) that includes a device the installed roster revoked and a roster device under a revoked `b64u:` identity. **Reuses a reachability-cache entry younger than ~20s rather than always dialing (#89)** — an unconditional probe collided with the `mcpmesh/ping/1` rate limiter, and a refused probe is reported as unreachable, so polling this verb faster than ~1/s made a healthy peer appear offline. Freshness is now the same contract `status` gives. **It never answers from a row that holds no pong** (`api_minor >= 66`, #215): a live session's path watcher writes a reachable row with no services in it, so such a row triggers a probe, and if that probe fetches nothing (the peer's limiter refused it) the verb fails with a retryable "could not be fetched just now" error instead of `[]`. Through 65 it answered `[]` from that row for up to ~20s after a session opened, indistinguishable from "offers you nothing"; from 66 an empty list always means the peer's pong named no services. **Answered from the LIVE service registry — the same one the accept path authorizes from (#100, `api_minor >= 17`). A service present in `config.toml` but not yet loaded is NOT reported: it would be refused on connect, which a caller cannot distinguish from a network failure. `status` likewise lists only live services.** | `{services:[…]}` |
...
| `user_key_export` | *(none)* — `api_minor >= 48`, #85 | `{recovery_phrase, user_id}` — the user key as 33 words a person can write down. **The phrase IS the private key**: anyone who reads it can present this identity. It is deliberately absent from the audit log, from `status`, and from every other surface — this response is the only place it exists. `user_id` is safe to display and record; compare it after an import. **Refused (`-32602`) on an ENROLLED device** (`api_minor >= 63`, #86, #219 — through 62 it returned the phrase of the local key): the local key `boot` minted is not the identity this node presents, so its phrase would restore a stranger to every peer — export from the device that holds the key. |
| `self_enroll_detach` | *(none)* — `api_minor >= 62`, #214 | `{user_id?, detached_from}` — LEAVE an identity this device was ENROLLED into (`pair { allow_self_enroll }`, #86): the adopted device→user binding is dropped live **and** on disk, and the node presents its own identity (imported, else boot-derived) again, returned as `user_id` for a UI to confirm. Idempotent on disk: a stale sidecar with nothing adopted live (an earlier import removed it best-effort, or boot declined a file that did not verify) is removed too, and the `-32058` message says so. The exit self-enrollment otherwise lacks — an enrolled device holds no user key, so `user_key_import` (the only other verb that clears the slot) is not available to it, and a person handed a substituted `mcpmesh-enroll:` line was a device of a stranger's identity until they deleted the state root by hand. **Local only:** it changes what this device presents, not what anyone believes — a peer that already learned this endpoint as that person's device is not told. Telling them is `device_revoke`, from the device that holds the key. Refused with `-32058` when nothing is adopted: nothing live is changed, but a stale enrollment file may be removed, and the message says so; a removal that fails is an uncoded `-32000` naming the file. Audited as a `trust` event (`self_enroll_detach`) keyed on the identity left. |
| `user_key_import` | `{recovery_phrase, replace?}` — `api_minor >= 48`, #85 | `{user_id, replaced}` — restore a user key on new hardware. On an ENROLLED device it supersedes the enrollment and removes the enrollment file **before** writing the key; if that file cannot be removed the import fails and changes nothing (`api_minor >= 62`, #214 — before, it warned and the next boot re-adopted the enrollment over the imported key). Whitespace and case are forgiven; a wrong word, the wrong count, or a failed checksum are refused **by position** rather than guessed at, because restoring a *different* identity looks exactly like every peer having forgotten you. Refuses to overwrite an existing key unless `replace` is set. The restored binding takes effect **immediately**, not at the next restart. **It does not get this device admitted** — peers authorize per device, so a recovered person still pairs (#85 ask 3 is unshipped). **In roster mode this desyncs you from the signed roster** — it pins the device→user binding against the user key that was current when the operator approved this device, and nothing re-signs; an operator must re-approve. `replace` is required only for a key the node LOADED — or one an earlier import wrote, in the same daemon lifetime too (`api_minor >= 63`, #221; through 62 a second import silently discarded the first) — not one its own boot minted seconds earlier, so a genuine new-laptop recovery does not need the destructive flag. |
...
- **An enrolled device cannot enroll a third** — it holds no private key. Enroll every device from
  ...
  place. For the same reason `peer_endorse`, `device_revoke` (#214) and `user_key_export` (#219)
  refuse (`-32602`) on an enrolled device: the local key underneath is not the identity it presents.
...
- **The enrolled device can leave, though** (`self_enroll_detach`, `api_minor >= 62`, #214). It
  drops the adopted binding and presents its own identity again — the exit for a device that
  redeemed a link it should not have, which before 62 meant deleting the state root by hand. This
  is the device's own exit, not a revocation: peers who already learned it as your device are not
  told, and the binding they hold still verifies. `device_revoke` from the key-holding device is
  what tells them.
...
`self_user_key_held` (`api_minor >= 62`, #214) is `true` when THIS device holds the private key
behind `self_user_id`. It is `false` — and elided — when the device presents that identity on the
strength of an ADOPTED enrollment binding (#86; the key lives on the device that enrolled it), and
when it has no user key at all. An enrolled device and a key-holding one otherwise report the same
`self_user_id`, and four things hinge on the difference: `peer_endorse`, `device_revoke` and
`invite { as_self }` all refuse on an enrolled device, and `self_enroll_detach` applies only there.
Advisory display data for placing those affordances, never an authorization input — every verb
re-checks. It reads `false` from a daemon below 62, which is that daemon's *wrong* answer for a
key-holder, so guard on `api_minor` before rendering it.

Each `recent_pairings` row carries `self_enroll: true` (`api_minor >= 62`, #214; elided when false)
when the ceremony that produced it was a SELF-ENROLLMENT rather than a pairing. See
[Embedding the pairing ceremony](#embedding-the-pairing-ceremony-both-sides-of-the-sas) for why an
inviter-side SAS panel must route on it.
...
`revoked_endpoints` (it is absent from `roster_members` too). Two consequences worth knowing: a `b64u:` entry hides for exactly as long as the
identity row stands (`api_minor >= 61`, #218) — `peer_unrevoke` on one device (by nickname) lifts
that endpoint's row and leaves the identity row, and the gate refuses the device on the identity
alone, so the grant stays hidden until `peer_unrevoke` names the `b64u:` itself (at 60 the entry
reappeared after a per-device unrevoke, which described a gate that never consulted the identity
table — the #218 defect); and a
bare roster name (a group, a roster `user_id`) is never hidden by this table — roster membership is
withdrawn by roster INSTALL, which rebuilds the view. The check fails CLOSED per entry: if the store cannot be read, the entry is
...
**Which ceremony produced the row matters (`api_minor >= 62`, #214).** A self-enrollment
(`invite { as_self: true }`) lands on the same ring, and its row carries `self_enroll: true`. Route
on that field, not on the row's `peer_nickname` (a display string the daemon owns and may reword).
The two ceremonies have two different mismatch remedies: for a pairing, "they don't match" means
`peer_remove` the new peer; for an enrollment there is **no peer row** to remove, and a mismatch
means an impostor has just been issued a binding for *your* identity — the remedy is
`device_revoke` of that endpoint, from this (key-holding) device. The row stays surface-clean and
carries no endpoint; the `trust` audit event `self_enroll` (on `audit_list` and live on
`subscribe`) names it as `target`, and it is the newest such event. Before 62 the only signal was
the nickname text, and that audit event carried no SAS to join on.
...
- **A peer that dies without closing** — a crash, power loss, a cut network — leaves the shared
  connection open on this side until the idle timeout (30 s by default). While another session to
  that peer still holds it, an `open_session` in that window no longer fails at open with `-32055`:
  it opens at once on the dead connection and then
...
>
> **Below 57 it was still on the session's FIRST frame**, whatever that frame's method was, plus any
> later `initialize`. So a backend that binds identity at session start was never blind, including
> under MCP `2026-07-28` where the first frame is an ordinary request. What was missing was frames
> 2..N: a backend that authorizes *per request* rather than at session start saw nothing on them.
> Guard on `>= 57` if that is you.
>
> That gap was fail-closed, never forgeable — the strip has run on every frame since `api_minor` 37,
> so the backend saw nothing rather than something caller-supplied.
>
> Two things are deliberately NOT attributed:
>
> - A **response** travelling caller→backend (the push direction, see "Server-initiated frames"). It
>   has no `method`; it carries `id` + `result`, and inventing a `params` object on one would be
>   malformed JSON-RPC.
> - A request with **positional (array) params**, or any non-object `params`. There is nowhere to put
>   `_meta` without deleting the caller's arguments, and this daemon pumps rather than rewrites — so
>   the frame passes through intact and un-attributed. MCP mandates object params, so this is the
>   non-MCP JSON-RPC backend case.
>
...
| `-32056` | `service_allow_grant` — the principal is **revoked** on this node, so the grant was refused **before anything was written** (#212, `api_minor >= 60`); also `peer_introduce` when the subject proves a `user_id` this node has revoked, and `pair` when the INVITER is revoked on this (the redeeming) node — refused before the dial for its endpoint or the identity its row carries (nothing contacted, invite untouched), after the exchange for the identity it proves in its reply (invite consumed) — with no row and no grant-back written either way (#218, `api_minor >= 61`). "Revoked" is what admission means by it, read live: an `eid:` that `peer_revoke` (or a signed import, or the installed roster) marked dead, or a `b64u:` identity `peer_revoke` revoked — every row carrying that `user_id` is refused, whatever the endpoint table says about the device (#218). Below 60 the grant succeeded and wrote an `allow` entry admission never honoured; below 61 the introduction installed a row admission refuses. Remedy: `peer_unrevoke` first if the revocation was a mistake; otherwise there is nothing to grant. `-32053`..`-32055` are skipped on purpose — they are the session-plane codes below. |
| `-32057` | `pair` — the INVITER refused because it has **revoked your identity** (`peer_revoke b64u:` on its side) (#218, `api_minor >= 61`). The inviter-side twin of `-32056`, and a different code because it names the *other* node's table: nothing on this side lifts it. **Do not render this as "ask for a fresh invite"** — the invite was consumed, and any invite redeemed by a device presenting that identity is refused the same way until the inviter runs `peer_unrevoke`. **What it is not:** identity revocation refuses a device *presenting* the identity; it cannot stop a person who stops presenting the key — redeeming with no binding (or under a new user key) lands as an ordinary `eid:` pairing of an endpoint the inviter never revoked. Below 61 the inviter made no such check: it wrote the row, said "paired", and refused every session from it. Only a device the inviter holds **no row** for (the person's next device) sees `-32057`: a device whose own row carries the revoked identity is closed before it presents the secret, exactly as an endpoint-revoked device is, and sees `-32045` with the invite untouched. |
| `-32058` | `self_enroll_detach` — nothing is adopted live; this device already presents its own identity (#214, `api_minor >= 62`). Nothing live was changed, but a stale enrollment file (one boot declined because it did not verify) may have been removed so the next boot cannot re-adopt it, and the message says when. A removal that fails is an uncoded `-32000` naming the file, not this code. Offer the detach only when `self_user_id` is present and `self_user_key_held` is `false`, and this code means a race (another detach, or a `user_key_import`): treat it as "already done". |
| `-32059` | `invite` — `as_self` on a device that was itself ENROLLED into another identity, so it holds no user key to sign a binding with (#214, `api_minor >= 62`). Refused **at mint**, on the device that can act on it; through 61 the mint succeeded and the redeemer got `-32049`. A permanent condition, not "try again": enroll from the device that holds the key, or `self_enroll_detach` this one first. `status.self_user_key_held` says it in advance. |
...
- **`api_version` = `"MAJOR.MINOR"`, and `api_minor` = the integer MINOR** — the
  ...
  An IDENTITY revocation holding at every site (#218) is `api_minor >= 61`: `pair` answers `-32057`
  when the inviter has revoked the redeemer's proven `user_id`, `peer_introduce` answers `-32056`
  for a subject proving one this node revoked, and admission — and every outbound dial
  (`open_session`, `peer_services`, `peer_diagnostics`, and `pair` redeeming from an inviter this
  node revoked, which answers `-32056`) — refuses any stored row carrying a revoked `user_id` — so a per-device `peer_unrevoke` under a standing identity revocation re-admits
  nothing, and a hidden `services[].allow` entry stays hidden until the identity itself is
  unrevoked (at 60 it reappeared). Guard on `>= 61` before branching on `-32057`.
  The self-enrollment exit and the surface an embedder needs to ship that ceremony (#214) are
  `api_minor >= 62`: the `self_enroll_detach` verb, `RecentPairing.self_enroll`,
  `StatusResult.self_user_key_held`, `-32058` / `-32059`, and two behaviour changes — `device_revoke`
  now refuses on an enrolled device instead of self-applying a revocation no peer will honour, and
  `invite { as_self }` on one is refused at mint. The two fields read `false` from an older daemon,
  which is that daemon's wrong answer for the case they exist to name, so guard before rendering.
  (Crate-level this is a MINOR release of `mcpmesh-node`: `InviterCtx::record_pairing`'s closure
  signature and `MeshState::recent_pairings`'s visibility changed.)
  The identity gates holding under the user-key lock (#221, #219) are `api_minor >= 63` — behaviour
  changes with no shape change: `user_key_export` refuses (`-32602`) on an enrolled device;
  `user_key_import`'s `replace` guard protects a key an earlier import wrote in the same daemon
  lifetime (through 62 a second import without `replace` discarded the first); `peer_endorse`
  refuses when no user key is on disk instead of minting one; and `peer_endorse`, `device_revoke`,
  `user_key_export` and the self-enrollment signature check the enrollment gate under the same lock
  an adoption or import writes under, so neither can land between the check and the signature.
  (Crate-level a MINOR release of `mcpmesh-node`: `pairing::rendezvous::SignBindingFn` became async.)
  Revocation holding on the OUTBOUND dial paths (#223) is `api_minor >= 64` — behaviour changes
