...
| `device_revoke` | `{endpoint, reason?}` — sign a **portable** revocation of one of **your own** devices with your user key. The direction local revocation cannot express: your peers cannot discover your laptop was stolen. Also applies it here. Requires a user key **that this device holds**: on a device ENROLLED into another identity (#86) it is refused with `-32602` before anything is applied, severed or audited (`api_minor >= 60`, #214) — through 59 it signed under the LOCAL key boot mints, applied the revocation here and returned a token no peer would accept, a silent partial success. Revoke from the device that holds the key. | `{token, endpoint, user_id}` |
...
- **The enrolled device can leave, though** (`self_enroll_detach`, `api_minor >= 60`, #214). It
  drops the adopted binding and presents its own identity again — the exit for a device that
  redeemed a link it should not have, which before 60 meant deleting the state root by hand. This
...
`self_user_key_held` (`api_minor >= 60`, #214) is `true` when THIS device holds the private key
...
re-checks. It reads `false` from a daemon below 60, which is that daemon's *wrong* answer for a
key-holder, so guard on `api_minor` before rendering it.

Each `recent_pairings` row carries `self_enroll: true` (`api_minor >= 60`, #214; elided when false)
...
**Which ceremony produced the row matters (`api_minor >= 60`, #214).** A self-enrollment
...
`subscribe`) names it as `target`, and it is the newest such event. Before 60 the only signal was
...
- **`api_version` = `"MAJOR.MINOR"`, and `api_minor` = the integer MINOR** — the
  ...
  `api_minor >= 60`: the `self_enroll_detach` verb, `RecentPairing.self_enroll`,
  `StatusResult.self_user_key_held`, `-32056` / `-32057`, and two behaviour changes — `device_revoke`
