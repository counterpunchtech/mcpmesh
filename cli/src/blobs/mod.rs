//! The gated per-scope app-blob provider (spec §9 / §11.2 P10 / §16 M4). A SEPARATE transport from
//! the M3c roster blob (`roster::transport::RosterBlobs`, MemStore, `iroh_blobs::ALPN`, ungated): an
...
/// The app-blob ALPN (spec §9's named `mcpmesh/blob/1` protocol), DISTINCT from `iroh_blobs::ALPN`
...
/// `iroh_blobs::ALPN`, `[RECONCILE-ALPN]`).
